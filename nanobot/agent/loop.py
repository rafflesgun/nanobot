...
class AgentLoop:
    ...
    def _ordered_fallback_models(self, primary_model: str) -> list[str]:
        """Return de-duplicated fallback models in configured order."""
        ordered: list[str] = []
        seen = {primary_model}
    ...
    def _is_fallback_eligible_error_str(error_msg: str) -> bool:
        """Return whether a provider error should try the next fallback model."""
        msg = error_msg.lower()
        return any(
            marker in msg
            for marker in (
    ...
    def _set_tool_context(
        ...
        """Support legacy direct callers without affecting scoped turn routing."""
        ...
        key = session_key or (
            ...
            else f"{channel}:{chat_id}:topic:{thread_id}"
    ...
    def _persist_user_message_early(
        ...
        **kwargs: Any,
        ...
        """
        if msg.metadata.get("_ephemeral_session"):
            return False
    ...
    def _request_context_for_turn(self, ctx: TurnContext) -> RequestContext:
        ...
        return RequestContext(
            ...
            thread_id=ctx.msg.metadata.get("message_thread_id"),
    ...
    async def _resolve_runtime_context_for_turn(
        ...
        blocks = await resolve_runtime_context(providers, ctx.request_context)
    ...
    def _handle_temp_command(
        ...
        if not temp_arg:
            ...
            if is_override:
                status = (
                    f"🌡️ Current temperature: `{effective_temp}`\n"
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        if model_override:
            runtime = dataclasses.replace(runtime, model=model_override)
        if temperature_override is not None:
            runtime = runtime.with_generation_overrides(temperature=temperature_override)
        request_ctx = request_context or RequestContext(
            ...
            thread_id=(metadata or {}).get("message_thread_id"),
        ...
        try:
            ...
            model_candidates = [runtime.model, *self._ordered_fallback_models(runtime.model)]
            for index, model_name in enumerate(model_candidates):
                attempt_runtime = runtime if model_name == runtime.model else dataclasses.replace(
                    runtime,
                    model=model_name,
                )
                try:
                    ...
                    if index + 1 < len(model_candidates) and self._is_fallback_eligible_error_str(str(exc)):
                        logger.warning(
                            "Model {} failed with provider error; trying fallback {}",
                            model_name,
                ...
                if (
                    ...
                    and index + 1 < len(model_candidates)
                    and self._is_fallback_eligible_error_str(result.error or result.final_content or "")
                ):
                    logger.warning(
                        "Model {} returned provider error; trying fallback {}",
                        model_name,
                        model_candidates[index + 1],
                    )
                    continue
    ...
    async def _dispatch(self, msg: InboundMessage) -> None:
        ...
        if session_key != msg.session_key and dataclasses.is_dataclass(msg):
            msg = dataclasses.replace(msg, session_key_override=session_key)
        lock = self._session_locks.setdefault(session_key, asyncio.Lock())
        gate_ctx = self._concurrency_gate or nullcontext()

        pending: asyncio.Queue | None = None
        try:
            async with lock, gate_ctx:
                if self._concurrency_gate is not None:
                    max_req = int(os.environ.get("NANOBOT_MAX_CONCURRENT_REQUESTS", "3"))
                    active = max_req - self._concurrency_gate._value
                    logger.debug(
                        "Entered agent concurrency gate (active={}/{})",
                        active,
                        max_req,
                    )
            ...
            if self._concurrency_gate is not None:
                ...
                active = max_req - self._concurrency_gate._value
                logger.debug(
                    "Released agent concurrency gate (active={}/{})",
                    active,
                    max_req,
                )
            if pending is None:
    ...
    async def _process_system_message(
        ...
        *,
        ...
        key = (msg.session_key_override if isinstance(msg.session_key_override, str)
               else f"{channel}:{chat_id}")
        session = self.sessions.get_or_create(key)
        ...
        if (thread_id := msg.metadata.get("message_thread_id")) is not None:
            outbound_metadata["message_thread_id"] = thread_id
    ...
    async def _state_restore(self, ctx: TurnContext) -> TurnState:
        ...
        if msg.media and isinstance(msg.media, list):
            new_content, image_only = self._prepare_message_media(msg.content, msg.media)
            if dataclasses.is_dataclass(msg):
                ctx.msg = dataclasses.replace(msg, content=new_content, media=image_only)
            else:
                ctx.msg = InboundMessage(
                    channel=getattr(msg, "channel", "system"),
                    sender_id=getattr(msg, "sender_id", "system"),
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        # Compatibility observability for legacy callers; tool execution uses
        # the request ContextVar bound by _run_agent_loop.
        if cron_tool := self.tools.get("cron"):
            set_context = getattr(cron_tool, "set_context", None)
            if callable(set_context):
                set_context(ctx.request_context)
        ...
        ctx.user_persisted_early = self._persist_user_message_early(
            ...
            runtime_context_blocks=[
                block for block in ctx.runtime_context_blocks if block.source != "channel_info"
            ],
        )
    ...
    def _set_runtime_checkpoint(self, session: Session, payload: dict[str, Any]) -> None:
        ...
        if session.metadata.get("_ephemeral"):
            return
