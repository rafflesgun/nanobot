...
class TelegramChannel(BaseChannel):
    ...
    def _start_typing(
        ...
        *,
        ...
        key = comp_key or str(chat_id)
        # Cancel any existing typing task for this chat
        self._stop_typing(key)
        target_chat_id = str(chat_id or key.split(":", 1)[0])
        self._typing_tasks[key] = asyncio.create_task(
            self._typing_loop(target_chat_id, thread_id=thread_id)
        )

    def _stop_typing(self, chat_id: str | None = None, *, comp_key: str | None = None) -> None:
    ...
    def _pick_react_emoji(self) -> str:
        """Return the configured ACK reaction emoji, supporting fixed or random lists."""
        configured = self.config.react_emoji
        if isinstance(configured, str):
            return configured
        choices = [emoji for emoji in configured if emoji]
        return random.choice(choices) if choices else ""

    async def _add_ack_reaction(self, chat_id: int | str, message_id: int) -> None:
        """Add the configured ACK reaction to an inbound message."""
        await self._add_reaction(str(chat_id), message_id, self._pick_react_emoji())
    ...
    async def _add_reaction(self, chat_id: str, message_id: int, emoji: str | list[str]) -> None:
        ...
        if isinstance(emoji, list):
            emoji = random.choice([item for item in emoji if item]) if any(emoji) else ""
