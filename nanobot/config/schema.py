...
class TTSConfig(Base):
    ...
    riva_function_id: str | None = None


class TelegramConfig(Base):
    """Telegram channel configuration compatibility schema."""
    ...
    stream_edit_interval: float = Field(default=1.0, ge=0.1)
    webhook_url: str = ""
...
class HeartbeatConfig(Base):
    ...
    keep_recent_messages: int = 8  # Number of recent heartbeat messages to retain
...
class WorkspaceRestrictionConfig(Base):
    ...
    def __bool__(self) -> bool:
        """Preserve boolean use at tool and channel configuration boundaries."""
        return self.enabled
...
class ToolsConfig(Base):
    ...
    @field_validator("restrict_to_workspace", mode="before")
    @classmethod
    def _normalize_workspace_restriction(cls, value: Any) -> Any:
