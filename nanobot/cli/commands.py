...
def serve(
    ...
    agent_loop = AgentLoop(
        ...
        restrict_to_workspace=runtime_config.tools.restrict_to_workspace.enabled,
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        context_block_limit=getattr(config.agents.defaults, "context_block_limit", None),
        max_tool_result_chars=getattr(config.agents.defaults, "max_tool_result_chars", 16_000),
        provider_retry_mode=getattr(config.agents.defaults, "provider_retry_mode", "standard"),
        web_search_config=config.tools.web.search,
        web_proxy=config.tools.web.proxy or None,
        exec_config=config.tools.exec,
        cron_service=cron,
        restrict_to_workspace=config.tools.restrict_to_workspace.enabled
        if hasattr(config.tools.restrict_to_workspace, "enabled")
        else config.tools.restrict_to_workspace,
        session_manager=session_manager,
        mcp_servers=config.tools.mcp_servers,
        channels_config=config.channels,
        timezone=getattr(config.agents.defaults, "timezone", "UTC"),
        extra_read=config.tools.restrict_to_workspace.extra_read
        if hasattr(config.tools.restrict_to_workspace, "extra_read")
        else None,
        extra_write=config.tools.restrict_to_workspace.extra_write
        if hasattr(config.tools.restrict_to_workspace, "extra_write")
        else None,
            ...
            if should_notify:
                ...
                if resp:
                    if job.payload.thread_id is not None:
                        ...
                        resp.metadata.setdefault("message_thread_id", job.payload.thread_id)
    ...
    async def on_heartbeat_execute(tasks: str) -> str:
        ...
        if hb_cfg.keep_recent_messages > 0:
            session = agent.sessions.get_or_create("heartbeat")
            session.retain_recent_legal_suffix(hb_cfg.keep_recent_messages)
            prune = getattr(session, "prune_by_token_limit", None)
            if callable(prune):
                prune(4000)
            else:
                legacy_prune = getattr(session, "prune_by_content_length", None)
                if callable(legacy_prune):
                    legacy_prune(4000)
