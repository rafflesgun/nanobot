...
describe('SubagentsPanel', () => {
  ...
  it('opens markdown editor from edit icon and saves workspace subagents', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.get('[data-testid="subagent-markdown"]').element).toBeTruthy())
  ...
  it('renders built-in subagents as read-only in editor', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.get('[data-testid="subagent-markdown"]').attributes('readonly')).toBeDefined())
    expect(wrapper.find('[data-testid="save-subagent"]').attributes('disabled')).toBeDefined()
  })
})
